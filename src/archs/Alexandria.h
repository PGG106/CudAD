 ...
 */
#ifndef CUDAD_SRC_ARCHS_ALEXANDRIA_H_
#define CUDAD_SRC_ARCHS_ALEXANDRIA_H_
...
#pragma omp parallel for schedule(static) num_threads(8)
    ...
    static int index(Square psq, Piece p, Square kingSquare, Color view) {
        return mirrorVertically(psq) +
               (getPieceType (p)         ) * 64 +
               (getPieceColor(p) == BLACK) * 64 * 6;
    }
...
#endif    // CUDAD_SRC_ARCHS_Alexandria_H_
